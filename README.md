...
## Notebook (`code.ipynb`)
This notebook implements a full quantitative research pipeline for testing the AQR "Quality" investment factor on a sample of U.S. equities. It loads and cleans monthly data on the Quality factor and its three sub-components (Profitability, Growth, Safety) alongside stock prices, market capitalization, and macroeconomic indicators from 1980 to 2022, then measures each factor's predictive power using Information Coefficient (Spearman correlation) analysis, tracking how that predictive power has evolved over four decades and by decade. It builds and backtests a 130/30 long-short portfolio strategy based on the Quality factor over 2005–2022 (via the `apmodule.ls_backtesting()` function, with monthly rebalancing and 0.2% transaction costs), examining performance through the 2008 financial crisis and COVID-19, and then optimizes the strategy by grid-searching portfolio breadth (100/250/400 stocks) and rebalancing frequency (1/3/6 months) to find the risk-adjusted-optimal configuration. It further tests whether the factor's predictive power is sensitive to the macroeconomic regime — inflation, interest rates, unemployment, consumer sentiment — using rolling correlations and regime-conditional IC analysis, and finally proposes and backtests two original alternatives to AQR's equal-weighted factor combination (an exponentially IC-weighted blend and a momentum-weighted blend of the three sub-components), comparing their cumulative performance against the original. Altogether, the notebook produces a complete, quantitatively grounded assessment of whether the Quality factor works, how its effectiveness has changed over time and across macro conditions, what the best-performing version of a tradable strategy on it looks like, and whether a smarter combination of its components can improve on AQR's original definition.
